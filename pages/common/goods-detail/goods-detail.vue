		...
		<view class="shop-box" @click="goShop(shop_info.shop_id)">
			<view class="shop">
				<view style="display: flex;align-items: center;" v-if="shop_info">
					<image :src="$src+'/images/shop-logo.png'" v-if="!shop_info.shop_logo"></image>
		...
		<view class="footer-box" style="padding:  24rpx;">
			<view class="footer">
				<view class="collect" style="background: #fff;position: relative;top: 8rpx;">
					...
					<!-- #ifdef H5 -->
					<view @click="share">
						<image :src="$src+'/images/fenxiang.png'" mode=""></image>
						<view>
							<text style="color: #797979">分享</text>
						</view>
					</view>
					<!-- #endif -->
					<!-- 出了H5端跟APP端均存在 -->
					...
					<view>
						...
						<image src="https://www.dxsc.vip/images/fenxiang.png" mode=""></image>
					...
					<!-- #endif -->
					<view @click="favorate()">
						<image v-if="is_favorate!=1" :src="$src+'/images/collect.png'" mode="" style="width: 50rpx;"></image>
						<image v-if="is_favorate==1" :src="$src+'/images/collect-active.png'" mode="" style="width: 50rpx;"></image>
				...
				<view class="show-info">
					<view class="show-title">
						...
						<view class="show-close" @click="close">
							<image :src="$src+'/images/error.png'" mode=""></image>
						</view>
					</view>
				</view>
			</view>
					...
					<block v-if="goodsParams && goodsParams.length > 0">
						<view class="params-list" v-for="(item,index) in goodsParams" :key="index">
							...
							<view class="params-item" v-if="item.value && item.value.length > 0">
								<text v-for="(list,index2) in item.value" :key="index2" :class="list.active?'paramsActive': ''" @click="chooseParams(index,index2)">
				...
				<view class="params-number">
					...
					<view class="number-item">
						<!-- <view class="item" style="border-right: 0;" @click="increment()">
							<image :src="$src+'/images/increment.png" mode=""></image>
						</view> -->
						<view class="item" style="border-right: 0;">
							<image :src="$src+'/images/decrement.png'" mode="" @click="decrement()"></image>
						</view>
		...
		methods: {
			...
			getGoodsInfo() {
				request({
					...
					res.data.goods_detail.description = res.data.goods_detail.description.replace(regex,
				...
				request({
					...
					if (res.data.code == 201) {
						...
						// 判断加入购物车或者购买的商品是否有库存
							...
							// 	return
							// }
							if(this.goodsParams.length > 0) {
								console.log(111)
								for(let i = 0; i < this.goodsParams.length; i++) {
									for(let j = 0; j < this.goodsParams[i].value.length; j++) {
										console.log(this.goodsParams[i].value[j].active)
										if(this.goodsParams[i].value[j].active) {
											if (this.dataList.stock == this.amount) {
												uni.showToast({
													title: '所选数量不能超过库存数',
													icon: 'none'
												})
												...
												if (!this.isType) {
													request({
														...
														method: 'post',
														data: {
															cart_detail: JSON.stringify(data),
															cart_tag: 'addCart'
														},
													}).then(res => {
														if (res.data > 0) {
			...
			onShareAppMessage() {
				return {
					...
					path: '/pages/common/goods-detail/goods-detail?id=' + this.id
				}
			},
			//#endif
			// App分享
			...
			share() {
				...
				uni.share({
				    ...
				    type: 0,
				    href: "https://dxsc.vip/pages/common/goods-detail/goods-detail?id="+this.id,
				    title: this.dataList.goods_name,
				    summary: "商家赚钱，消费者省钱的平台",
				    imageUrl: "https://www.dxsc.vip/images/shop-logo.png",
